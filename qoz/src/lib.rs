...
/// Convert from Gray, GrayAlpha, RGB pixels to RGBA pixels.
#[inline]
pub fn to_rgba(
  pixels: Vec<u8>,
  width: u32,
  ...
  // already in RGBA. just return. handling early to avoid unnecessary vec allocation.
  ...
  pixels
    .par_chunks((channels as u32 * width) as usize)
    .zip(rgba_pixels.par_chunks_mut(4 * width as usize))
    .for_each(|(src_row, dst_row)| match channels {
      Channels::Gray => dst_row
        .as_chunks_mut::<4>()
        .0
        .iter_mut()
        .zip(src_row.as_chunks::<1>().0.iter())
        .for_each(|(d, s)| {
          d[0] = s[0];
          d[1] = s[0];
          d[2] = s[0];
...
pub fn encode(
  ...
  // create a buffer, encode into it and trim it.
  ...
  let true_lens: Result<Vec<usize>, QozError> = tile_slices
    ...
    .map_init(
      ...
      |compressor_res, (tile, out_slice)| match compressor_res {
        ...
        Err(e) => Err(QozError::Zstd(std::io::Error::other(format!(
          "zstd initialization failed: {e}"
        )))),
      },
    )
    .collect();
