...
fn set_last_error(msg: impl std::fmt::Display) {
  ...
  let c = CString::new(msg)
    .unwrap_or_else(|_| CString::new("qoz: error message contained an interior NUL byte").unwrap());
...
/// `Err` or a panic, and clears the last error and returns the value on `Ok`.
...
pub extern "C" fn qoz_max_encoded_len(
  ...
  match Channels::try_from(channels) {
    Ok(c) => qoz::encode_max_len(width, height, c, tile_rows),
    Err(_) => 0,
  }
}
